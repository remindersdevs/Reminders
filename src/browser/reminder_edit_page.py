...
@Gtk.Template(resource_path='/io/github/retainerdevs/Retainer/ui/reminder_edit_page.ui')
class ReminderEditPage(Gtk.Box):
    __gtype_name__ = 'ReminderEditPage'
    ...
    def __init__(self, win, app, **kwargs):
        ...
        self.win.add_shortcut(Gtk.Shortcut.new(Gtk.ShortcutTrigger.parse_string('<Ctrl>s'), Gtk.CallbackAction.new(lambda *args: self.on_save())))
    ...
    def focus(self):
        self.win.leaflet.navigate(Adw.NavigationDirection.FORWARD)
        self.title_entry.grab_focus()

    def unfocus(self, check_changed = True):
        self.win.leaflet.navigate(Adw.NavigationDirection.BACK)
        if check_changed:
            self.setup(None)
        else:
            self.do_setup(None)
    ...
    def setup(self, reminder = None, new = False, force = True):
        if self.check_changed(self.get_options()):
            if force:
                confirm_dialog = Adw.MessageDialog(
                    transient_for=self.win,
                    heading=_('You have unsaved changes'),
                    body=_('Are you sure you want to abandon them?'),
                )
                ...
                confirm_dialog.connect('response::yes', lambda *args: self.do_setup(reminder, new))

                confirm_dialog.present()
        else:
            self.do_setup(reminder, new)

    def do_setup(self, reminder = None, new = False):
        ...
        if not new:
            self.page_label.set_label(_('Edit Reminder'))
        else:
            self.page_label.set_label(_('New Reminder'))
                    ...
                    GLib.Variant(
                        ...
                        (
                            info.app_id,
                            {
                                ...
                                'due-date': GLib.Variant('t', options['due-date']),
                                'timestamp': GLib.Variant('t', options['timestamp']),
                                'important': GLib.Variant('b', options['important']),
                                ...
                                'repeat-until': GLib.Variant('t', options['repeat-until']),
